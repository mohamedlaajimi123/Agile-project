...
export default function StudentTopbar({ isDark, t, searchQuery, setSearchQuery, notificationCount, programName }) {
  return (
    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8 flex items-center justify-between">
      <div>
        ...
        <p className={isDark ? 'text-gray-400' : 'text-slate-600'}>{programName}</p>
      </div>
